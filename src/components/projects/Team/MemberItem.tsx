...
export default function MemberItem({ member, userId, managerId }: MemberItemProps) {
   ...
   const { mutate } = useMutation({
      ...
      onError: (error) => {
         toast.error(
            error.message === "The User is not a member of team" &&
         ...
         <div className="flex flex-col">
            <p className="text-3xl mb-2 font-semibold hover:translate-x-5 hover:scale-110 hover:opacity-95 duration-300 flex items-center">
               ...
               {managerId === member._id && (
                  <span className="text-sm ml-2 px-4 uppercase bg-green-300 text-green-700 border border-green-700 rounded-full">
                     Manager
                  </span>
               )}
            </p>
